...
public class TOP_deterministicVersion6 {
	public static void solveMe(Inputs inp, Test t) {
		...
		try {
			...
			IloNumVar[][] lenght=new IloNumVar[V+1][K];
			...
			for(int d=0;d<inp.getVehNumber();d++) {
				for(int j= 1; j<n;j++) {
					expr7.addTerm(1.0, x[0][j][d]);
					cplex.addLe(expr7, inp.getVehNumber());
				}
			}
			/********************/
			...
			for(int d=0;d<inp.getVehNumber();d++) {
				for(int o=1; o<(n-1);o++) {
					...
					expr3.addTerm(1.0, x[0][o][d]);
					expr3.addTerm(-1.0, x[o][n-1][d]);
					cplex.addEq(expr3, 0); 
				}
